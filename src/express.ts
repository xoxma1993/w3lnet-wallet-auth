...
export function createWalletAuthRouter(options: CreateWalletAuthRouterOptions): ExpressRouter {
  ...
  router.post("/verify", async (request, response, next) => {
    try {
      const body = authVerifySchema.parse(request.body) as AuthVerifyRequest;
...
export function createApprovedSession(input: { subject: string; namespaces: AuthVerifyRequest["approvedNamespaces"]; issuedAt: Date; expiresAt: Date }): ApprovedSession {
  return {
    id: randomUUID(),
