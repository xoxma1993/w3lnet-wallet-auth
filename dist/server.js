...
const chainIdSchema = z.custom((value) => typeof value === "string" && /^(eip155|tron):[^:]+$/.test(value));
const caip10AccountSchema = z.custom((value) => typeof value === "string" && /^(eip155|tron):[^:]+:.+$/.test(value));
...
export function createNonceMessage(options) {
...
export function verifyTronMessage(options) {
...
export function validateApprovedNamespaces(options) {
    ...
    for (const [namespace, requirement] of Object.entries(proposal.requiredNamespaces)) {
        const approved = options.request.approvedNamespaces[namespace];
        if (!approved) {
    ...
    for (const [namespace, approved] of Object.entries(options.request.approvedNamespaces)) {
        const requirement = requested[namespace];
        if (!requirement) {
